...
export default {
  ...
  methods:{
      ...
      addAddress(){
        ...
        //  let tel=/^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/
        if(!this.phone|| this.phone.length != 11){
            this.confrim="请输入正确的电话格式";
            this.toast=true;
            return false;
        }
        if(!this.address[0]){
        ...
        API.person.addressAdd({
            ...
            if(res.body.code==200){
                ...
                if(this.$route.query.chose == 1) {
                    ...
                    this.$store.dispatch('SetAddress', res.body.data);
                    localStorage.setItem('addressClick','11');
                    /* 路由回退 */
                  if (this.$route.query.jiaokuaidi == 1) {
                    this.$router.push(`/jiaokuaidi?orderId=${this.$route.query.orderId}&address=${JSON.stringify(res.body.data)}`)
                  } else {
                    this.$router.push({
