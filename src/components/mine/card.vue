...
<div>
  <div class="help_common_title">
    <x-header @on-click-back="routerBack" :left-options="{backText: '',preventGoBack:true}">优惠券</x-header>
  </div>
    <div class="card_main">
            ...
            <div class="nondata" v-show="cardList.length==0">
                ...
                <li class="card_main_single" :class="{'card_main_single--select':!typeList[0].click}" v-for="item in cardList" @click="goShop(item)">
                        ...
                        <img v-if="!typeList[0].click"src="../../assets/img/common/closecard.png" alt="card">
                            ...
                            <div class="card_main_single_right">
                                <template v-if="item.use_range==1">
                                    <h3 class="card_main_single_title" v-if='item.goods_type===0'> <span style="font-size: 12px;position:relative;top:-3px;">博鸟红包{{item.amount}}元</span> </h3>
                                    <h3 class="card_main_single_title" v-else>{{cardtext[item.goods_type]}}</h3>
                                </template>
                                <template v-else>
  ...
  methods:{
      ...
      goShop(item){
          if(this.typeList[0].click){
              ...
              }else if(item.goods_type===0 || item.goods_type===1 ){
                this.$router.push({
                    path:"/"
                });
              }else{
