        ...
        if (isWeiXin()) {
            ...
            if (!openId) {
                if (access_code == null) {
                    ...
                    wx.miniProgram.getEnv(function(res) {if(res.miniprogram){location.href = url;}});
                } else {
                    if (!openId) {
                        API.order.getOpenId({
                            code: access_code,
            ...
            window.title="博鸟绘本-上博鸟绘本，租你所有！";
            window.imgurl = "http://oss.zujiekeji.cn/img/logo_120.png";
            window.desc = "博鸟绘本：一款专注于儿童用品领域的专业共享租赁APP，致力让共享成为一种的新的生活方式。";
            API.card.wxShare({
                url: link,
            }).then((data) => {
                wx.config({
